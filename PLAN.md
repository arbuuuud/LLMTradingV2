...
- [ ] **T5-3**: **Kalibrasi Spesifik Engine PAC via Naruto-2 Kage Bunshin** (Berdasarkan Hasil Pembelajaran 52 Forward Trades VPS):
  ...
  - [x] **Subtask 5-3E: Fail-Safe Resilience & Disconnection Protection Plan (Python Brain Crash / Network Partition)**:
    ...
    - *Arsitektur Solusi & Rencana Aksi (Fail-Safe Resilience Protocol)*:
      ...
      2. **MT5 Heartbeat Watchdog & Safety Neutralizer**: Jika koneksi socket TCP ke Python terputus lebih dari 10 detik, EA MT5 secara otomatis membatalkan semua Pending Limit Orders yang belum tersentuh (*Cancel Untouched Limits*) agar tidak tersambar spike liar saat tidak diawasi.
      ...
      4. **Atomic SQLite/Append-Only WAL Persistence**: Seluruh trade events di Python disimpan menggunakan format atomik tahan crash (`.tmp` + `os.replace`) sehingga jika proses terhenti di tengah penulisan, file log tidak akan pernah korup.
    - *Status*: **VERIFIED & COMPLETED (Implemented & Tested via Unit Tests + Live Simulator)**.
  - [x] **Subtask 5-3F: Multi-Account Dynamic Fleet Management (PropFirm, SweetSpot, Aggressive, YOLO)**:
    - *Latar Belakang Kebutuhan*: Menguji 4 profil risiko sekaligus dalam 1 instance Python Bridge server melayani kloning 4 terminal MT5 di VPS.
    - *Arsitektur Terimplementasi*:
      1. Auto-registration MT5 account dengan default status `risk_profile: none` dan `active: false` (safety-first standby).
      2. Kontrol visual interaktif di Dashboard tab **Accounts**: Dropdown 4 profil + tombol toggle saklar `ACTIVE / INACTIVE`.
